...
const WidgetManagementPlaceholder = () => {
  ...
  const renderPreviewWidget = () => {
    // Create a preview HTML page that loads the actual widget.js
    const previewHTML = `
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .demo-btn.primary {
            background: ${widgetConfig.theme.primaryColor};
            color: white;
        }
        ...
        @media (max-width: 768px) {
            ...
            .demo-content h1 { font-size: 2rem; }
...
<body>
    <div class="demo-content">
        <h1>${widgetConfig.content.companyName}</h1>
        <p>${widgetConfig.content.welcomeMessage}</p>
        <div class="demo-buttons">
            <button class="demo-btn primary" onclick="if(window.ShivAIWidget) window.ShivAIWidget.open()">
                💬 Start Chat
            </button>
            <button class="demo-btn secondary" onclick="if(window.ShivAIWidget && window.ShivAIWidget.toggleMode) window.ShivAIWidget.toggleMode()">
                📞 Voice Call
            </button>
        ...
        <div style="margin-top: 40px; padding: 20px; background: #f8fafc; border-radius: 8px; text-align: left;">
            <h3 style="margin: 0 0 10px 0; color: #374151; font-size: 1rem;">Preview Mode: ${previewDevice.charAt(0).toUpperCase() + previewDevice.slice(1)}</h3>
            <p style="margin: 0; color: #6b7280; font-size: 0.9rem;">This is a live preview of your widget using the actual widget.js file. Click the widget button to test functionality.</p>
        </div>
    </div>
    ...
    return (
      <iframe
        key={`${JSON.stringify(widgetConfig)}-${previewDevice}`}
        src={url}
