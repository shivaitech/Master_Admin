  ...
  const AIEmployeeStatsMain = ({ onViewEmployee }) => {
      ...
      try {
        ...
        if (response?.data?.success && responseData?.sessions) {
          ...
          const transformedSessions = sessionsArray.map((session, index) => ({
            ...
            callId: session.call_id || session.callId || session.session_id || session.id,
    ...
    const handleViewSession = async (session) => {
      ...
      console.log('🔍 Fetching call summary for agent ID:', agentId, 'and call ID:', callId);
    ...
    const fetchCallSummary = async (agentId, callId) => {
      ...
      console.log('📡 Fetching call summary for agent:', agentId, 'callId to filter:', callId);
                    ...
                    >
                      ...
                      {session.tokenUsage && (
                        <div className={`mt-3 pt-3 border-t ${currentTheme.border}`}>
                          <p className={`text-xs ${currentTheme.textSecondary} mb-2`}>Token Usage</p>
                          <div className="grid grid-cols-3 gap-2 sm:gap-4">
                            <div className={`${currentTheme.searchBg} rounded-lg p-2 text-center`}>
                              <p className={`text-xs ${currentTheme.textSecondary} mb-1`}>Input</p>
                              <p className={`${currentTheme.text} text-sm font-bold`}>
                                {(session.tokenUsage.input_tokens || session.tokenUsage.inputTokens || 0).toLocaleString()}
                              </p>
                            </div>
                            <div className={`${currentTheme.searchBg} rounded-lg p-2 text-center`}>
                              <p className={`text-xs ${currentTheme.textSecondary} mb-1`}>Output</p>
                              <p className={`${currentTheme.text} text-sm font-bold`}>
                                {(session.tokenUsage.output_tokens || session.tokenUsage.outputTokens || 0).toLocaleString()}
                              </p>
                            </div>
                            <div className={`${currentTheme.searchBg} rounded-lg p-2 text-center`}>
                              <p className={`text-xs ${currentTheme.textSecondary} mb-1`}>Total</p>
                              <p className="text-blue-600 text-sm font-bold">
                                {(session.tokenUsage.total_tokens || session.tokenUsage.totalTokens || 0).toLocaleString()}
                              </p>
                            </div>
                          </div>
                        </div>
                          ...
                          {selectedSession.transcripts && selectedSession.transcripts.length > 0 ? (
                            <>
                              <p className={`text-xs sm:text-sm ${currentTheme.textSecondary} mb-2`}>
                                Conversation started at {formatDate(selectedSession.startTime)}
                              </p>
                                    ...
                                    <>
                                      ...
                                      <div className="flex items-start gap-1.5 sm:gap-2">
                                        ...
                                        <div className={`max-w-[80%] ${currentTheme.cardBg} rounded-2xl rounded-tl-sm px-3 py-2 sm:px-4 sm:py-3 border ${currentTheme.border}`}>
                                          <p className={`text-xs sm:text-sm ${currentTheme.text}`}>{message.message}</p>
                                        </div>
                                      </div>
                                    </>
                                  ) : (
                                      ...
                                      <div className="flex justify-end">
                                        <div className="flex flex-col items-end max-w-[80%]">
                                          <div className="rounded-2xl rounded-tr-sm px-3 py-2 sm:px-4 sm:py-3 bg-gradient-to-br from-blue-500 to-blue-600">
                                            <p className="text-xs sm:text-sm text-white">{message.message}</p>
                                          </div>
                                        </div>
                                      </div>
                                    </>
                        ...
                        {summaryLoading ? (
                          ...
                          (() => {
                            const filteredLeads = callSummary.data.leads.filter(lead => 
                              lead.callId === (selectedSession.callId || selectedSession.sessionId || selectedSession.id)
                            );
                            
                            console.log('🔍 Filtering leads. Looking for callId:', selectedSession.callId || selectedSession.sessionId || selectedSession.id);
                            ...
                            return filteredLeads.length > 0 ? (
                              ...
                              <div className="text-center py-6">
                                ...
                                <p className={`text-xs ${currentTheme.textSecondary} mt-1`}>
                                  Call ID: {selectedSession.callId || selectedSession.sessionId || selectedSession.id}
                                </p>
                              </div>
                            );
                          })()
